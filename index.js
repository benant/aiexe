...
(async () => {
    ...
    let php_interpreter;
    try {
        php_interpreter = await which_php();
    } catch { }
    let bash_interpreter;
    try {
        bash_interpreter = await which_bash();
    } catch { }
    let c_interpreter;
    try {
        c_interpreter = await which_c();
    } catch { }
    let java_interpreter;
    try {
        java_interpreter = await which_java();
    } catch { }
    // let javac_interpreter;
    // try {
    //     javac_interpreter = await which_javac();
    // } catch { }
    let node_interpreter;
    try {
        node_interpreter = await which_node();
    } catch { }
    // let jshint_interpreter;
    // try {
    //     jshint_interpreter = await which_jshint();
    // } catch { }
    ...
    async function disableAllVariable() {
        const variables = ['GOOGLE_API_KEY', 'OPENAI_API_KEY', 'GROQ_API_KEY', 'USE_LLM', 'ANTHROPIC_API_KEY', 'PYTHON_VENV_PATH', 'OLLAMA_PROXY_SERVER', 'OLLAMA_MODEL', 'OPENAI_MODEL', 'GROQ_MODEL', 'ANTHROPIC_MODEL','SELECTED_LANGUAGE'];
    ...
    async function which_python() {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            if (!ppath) continue;
            // if (ppath.indexOf(' ') > -1) throw ppath; // C:\Program Files\Git\usr\bin\bash.exe 처럼 윈도우 경로에는 빈문자열이 있을 수 있음. 왜 들어가 있는지 모르겠음.
            let str = `${Math.random()}`;
            let { stdout } = await execAdv(`${ppath} -c "print('${str}')"`, false);
            if (stdout.trim() === str) return ppath;
        }
    }
    async function which_php() {
        let list = ['php'];
        for (let i = 0; i < list.length; i++) {
            let name = list[i];
            let ppath = await which(name);
            ...
            let str = `${Math.random()}`;
            let { stdout } = await execAdv(`${ppath} -r "echo'${str}';"`, false);
            if (stdout.trim() === str) return ppath;
        }
    }
    async function which_bash() {
        let list = ['bash'];
        for (let i = 0; i < list.length; i++) {
            let name = list[i];
            let ppath = await which(name);
    ...
    function doctorCheck(display = false) {
        ...
        if (!SELECTED_LANGUAGE) {
            if (display) console.error(chalk.yellowBright(' - The "SELECTED_LANGUAGE" environment variable is missing but required.'));
            doctorOk = false;
        }
        if (!python_interpreter && SELECTED_LANGUAGE==='python') {
            if (display) console.error(chalk.yellowBright(" - Python is required to use this app."));
            doctorOk = false;
        }
        if (!php_interpreter && SELECTED_LANGUAGE==='php') {
            if (display) console.error(chalk.yellowBright(" - PHP is required to use this app."));
            doctorOk = false;
        }
        if (!bash_interpreter && SELECTED_LANGUAGE==='bash') {
            if (display) console.error(chalk.yellowBright(" - BASH is required to use this app."));
            doctorOk = false;
        }
        if (!c_interpreter && SELECTED_LANGUAGE==='c') {
            if (display) console.error(chalk.yellowBright(" - gcc is required to use this app."));
            doctorOk = false;
        }
        if (!java_interpreter && SELECTED_LANGUAGE==='java') {
            if (display) console.error(chalk.yellowBright(" - java is required to use this app."));
            doctorOk = false;
        }
        // if (!javac_interpreter && SELECTED_LANGUAGE==='java') {
        //     if (display) console.error(chalk.yellowBright(" - javac is required to use this app."));
        //     doctorOk = false;
        // }
        if (!node_interpreter && SELECTED_LANGUAGE==='nodejs') {
            if (display) console.error(chalk.yellowBright(" - node is required to use this app."));
            doctorOk = false;
        }
        // if (!jshint_interpreter && SELECTED_LANGUAGE==='nodejs') {
        //     if (display) console.error(chalk.yellowBright(" - jshint is required to use this app. \n - use this command 'npm install -g jshint'"));
        //     doctorOk = false;
        // }
        if (!perl_interpreter && SELECTED_LANGUAGE==='perl') {
            if (display) console.error(chalk.yellowBright(" - perl is required to use this app."));
            doctorOk = false;
        }
        if (!USE_LLM) {
    ...
    async function shell_exec(python_code) {
        switch(SELECTED_LANGUAGE) {
            case 'c':
                return await shell_exec_c(python_code);
            case 'java':
                return await shell_exec_java(python_code);
            case 'nodejs':
                return await shell_exec_nodejs(python_code);
            case 'batch':
                return await shell_exec_batch(python_code);
            case 'bash':
    ...
    async function shell_exec_c(c_code) {
        let response = await new Promise(resolve => {
            let scriptPath = `${PYTHON_VENV_PATH}/._code.bat`;
            let warninglist = [];
            let modulelist = [];
            let c_code_ = c_code;
            fs.writeFileSync(scriptPath, c_code_);
            let child;
            if (isWindows()) {
                let activateCmd = `${PYTHON_VENV_PATH}\\Scripts\\Activate.ps1`;
                // const nodejsInterpreterPath = [
                //     `${PYTHON_VENV_PATH}\\Scripts\\node.exe`,
                //     nodejs_interpreter
                // ].find(fs.existsSync);
                // const nodejs_interpreter_ = nodejsInterpreterPath || '';
                // if (!nodejs_interpreter_) throw new Error('nodejs Interpreter Not Found');
                // let nodejsCmd = `${nodejs_interpreter_} '${scriptPath}'`;
                child = shelljs.exec(`powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& {& '${activateCmd}'}; gcc ${scriptPath}; ${scriptPath}" < nul`, { async: true, silent: true });
            } else {
                // const nodejsInterpreterPath = [
                //     `${PYTHON_VENV_PATH}/bin/node`,
                //     nodejs_interpreter
                // ].find(fs.existsSync);
                // const nodejs_interpreter_ = nodejsInterpreterPath || '';
                // if (!nodejs_interpreter_) throw new Error('nodejs Interpreter Not Found');
                child = shelljs.exec(`"${nodejs_path}" -c "source "${PYTHON_VENV_PATH}/bin/activate" && "gcc "${scriptPath}" && "${scriptPath}" < /dev/null"`, { async: true, silent: true });
            }
            let stdout = [];
            let stderr = [];
            child.stdout.on('data', function (data) {
            ...
            child.on('close', function (code) {
                stdout = stdout.join('');
                stderr = stderr.join('');
                resolve({ code, stdout, stderr, java_code });
            });
        });
        return response;
    }
    async function shell_exec_java(java_code) {
        let response = await new Promise(resolve => {
            let scriptPath = `${PYTHON_VENV_PATH}/._code.java`;
            let warninglist = [];
            let modulelist = [];
            let java_code_ = java_code;
            fs.writeFileSync(scriptPath, java_code_);
            let child;
            if (isWindows()) {
                let activateCmd = `${PYTHON_VENV_PATH}\\Scripts\\Activate.ps1`;
                // const nodejsInterpreterPath = [
                ...
                // if (!nodejs_interpreter_) throw new Error('nodejs Interpreter Not Found');
                // let nodejsCmd = `${nodejs_interpreter_} '${scriptPath}'`;
                let nodejsCmd = `java ${scriptPath}`;
                child = shelljs.exec(`powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& {& '${activateCmd}'}; ${nodejsCmd}" < nul`, { async: true, silent: true });
            } else {
                // const nodejsInterpreterPath = [
                //     `${PYTHON_VENV_PATH}/bin/node`,
                //     nodejs_interpreter
                ...
                // const nodejs_interpreter_ = nodejsInterpreterPath || '';
                // if (!nodejs_interpreter_) throw new Error('nodejs Interpreter Not Found');
                child = shelljs.exec(`"${nodejs_path}" -c "source "${PYTHON_VENV_PATH}/bin/activate" && "java "${scriptPath}" < /dev/null"`, { async: true, silent: true });
            }
            let stdout = [];
            let stderr = [];
            child.stdout.on('data', function (data) {
    ...
    async function shell_exec_nodejs(nodejs_code) {
        let response = await new Promise(resolve => {
            let scriptPath = `${PYTHON_VENV_PATH}/._code.js`;
            let warninglist = [];
            let modulelist = [];
            let nodejs_code_ = nodejs_code;
            fs.writeFileSync(scriptPath, nodejs_code_);
            let child;
            if (isWindows()) {
                let activateCmd = `${PYTHON_VENV_PATH}\\Scripts\\Activate.ps1`;
                // const nodejsInterpreterPath = [
                ...
                // const nodejs_interpreter_ = nodejsInterpreterPath || '';
                // if (!nodejs_interpreter_) throw new Error('nodejs Interpreter Not Found');
                child = shelljs.exec(`"${nodejs_path}" -c "source "${PYTHON_VENV_PATH}/bin/activate" && "node "${scriptPath}" < /dev/null"`, { async: true, silent: true });
            }
            let stdout = [];
            let stderr = [];
            child.stdout.on('data', function (data) {
            ...
            child.on('close', function (code) {
                stdout = stdout.join('');
                stderr = stderr.join('');
                resolve({ code, stdout, stderr, nodejs_code });
            });
        });
        return response;
    }
    async function shell_exec_batch(bash_code) {
        let response = await new Promise(resolve => {
            let scriptPath = `${PYTHON_VENV_PATH}/._code.bat`;
            let warninglist = [];
            let modulelist = [];
            let bash_code_ = bash_code;
            fs.writeFileSync(scriptPath, bash_code_);
            let child;
            if (isWindows()) {
                let activateCmd = `${PYTHON_VENV_PATH}\\Scripts\\Activate.ps1`;
                let bashCmd = `${scriptPath}`;
                ...
                throw new Error('Batch must run on MS Windows.');
            }
            let stdout = [];
            let stderr = [];
            child.stdout.on('data', function (data) {
            ...
            child.on('close', function (code) {
                stdout = stdout.join('');
                stderr = stderr.join('');
                resolve({ code, stdout, stderr, bash_code });
            });
        });
        return response;
    }
    async function shell_exec_bash(bash_code) {
        let response = await new Promise(resolve => {
            let scriptPath = `${PYTHON_VENV_PATH}/._code.sh`;
            let warninglist = [];
            let modulelist = [];
            let bash_code_ = bash_code;
            fs.writeFileSync(scriptPath, bash_code_);
            ...
            if (isWindows()) {
                ...
                const bashInterpreterPath = [
                    `${PYTHON_VENV_PATH}\\Scripts\\bash.exe`,
                    bash_interpreter
                ].find(fs.existsSync);
                ...
                const bash_interpreter_ = bashInterpreterPath || '';
                if (!bash_interpreter_) throw new Error('BASH Interpreter Not Found');
                child = shelljs.exec(`"${bash_path}" -c "source "${PYTHON_VENV_PATH}/bin/activate" && "${bash_interpreter_}" -u "${scriptPath}" < /dev/null"`, { async: true, silent: true });
            }
            let stdout = [];
            let stderr = [];
            child.stdout.on('data', function (data) {
    ...
    async function shell_exec_php(php_code) {
        let response = await new Promise(resolve => {
            ...
            let warninglist = [];
            let modulelist = [];
            let php_code_ = php_code;
            fs.writeFileSync(scriptPath, php_code_);
            let child;
            if (isWindows()) {
                let activateCmd = `${PYTHON_VENV_PATH}\\Scripts\\Activate.ps1`;
                const phpInterpreterPath = [
                    `${PYTHON_VENV_PATH}\\Scripts\\php.exe`,
                    php_interpreter
                ].find(fs.existsSync);
                ...
                const php_interpreter_ = phpInterpreterPath || '';
                if (!php_interpreter_) throw new Error('PHP Interpreter Not Found');
                child = shelljs.exec(`"${bash_path}" -c "source "${PYTHON_VENV_PATH}/bin/activate" && "${php_interpreter_}" -u "${scriptPath}" < /dev/null"`, { async: true, silent: true });
            }
            let stdout = [];
            let stderr = [];
            child.stdout.on('data', function (data) {
            ...
            child.on('close', function (code) {
                stdout = stdout.join('');
                stderr = stderr.join('');
                resolve({ code, stdout, stderr, php_code });
            });
        });
        return response;
    }
    async function shell_exec_perl(perl_code) {
        let response = await new Promise(resolve => {
            ...
            if (isWindows()) {
                ...
                const perl_interpreter_ = perlInterpreterPath || '';
                if (!perl_interpreter_) throw new Error('Perl Interpreter Not Found');
                child = shelljs.exec(`"${bash_path}" -c "source "${PYTHON_VENV_PATH}/bin/activate" && "${perl_interpreter_}" -u "${scriptPath}" < /dev/null"`, { async: true, silent: true });
            }
            let stdout = [];
            let stderr = [];
            child.stdout.on('data', function (data) {
            ...
            child.on('close', function (code) {
                stdout = stdout.join('');
                stderr = stderr.join('');
                resolve({ code, stdout, stderr, perl_code });
            });
        });
        return response;
    }
    async function shell_exec_python(python_code) {
        let response = await new Promise(resolve => {
    ...
    function combindMessageHistory(summary, messages_, history, askforce) {
        return [
            askforce === 'ask_opinion' ? {
                ...
                role: "system",
                content: [
                    `# Create ${SELECTED_LANGUAGE} code to handle user requests`,
                    `You are a ${SELECTED_LANGUAGE} programmer who creates ${SELECTED_LANGUAGE} code to solve user's request.  `,
                    `The user will run the ${SELECTED_LANGUAGE} code you will provide, so you should only provide ${SELECTED_LANGUAGE} code that can be executed.  `,
                    ``,
                    `## INSTRUCTION:`,
                    `- Response only the ${SELECTED_LANGUAGE} code.`,
                    ...
                    `- Please avoid using commands that only work in interactive environments like Jupyter Notebooks, especially those starting with \`!\`, in standard ${SELECTED_LANGUAGE} script files.`,
                    `- Always use the explicit output method via the print function, not expression evaluation, when your ${SELECTED_LANGUAGE} code displays results.`,
                    ...
                    `${isWindows() ? `The ${SELECTED_LANGUAGE} code will run on Microsoft Windows Environment\n` : ''}`,
                    `## CODING CONVENTIONS:`,
                    `- STANDARD CODING BLOCK STYLE:`,
                    `    Write the coding block as follows:`,
                    `    ${threeticks}${SELECTED_LANGUAGE}`,
                    `    (write code here)`,
                    `    ${threeticks}`,
                    `- STANDARD CODING STYLE TO IMPORT:`,
                    `    If You Want to Use a Module in Your Code on Python, Import the Module Using the following Logic before Using It.`,
                    `    ${threeticks}python`,
                    `    try:`,
                    `        import package_name`,
                    `    except ImportError:`,
                    ...
                    ``,
                    `## Exception`,
                    `- As an exception, if you request a simple explanation that does not require ${SELECTED_LANGUAGE} code to resolve the user's request, please respond with an explanation in natural language rather than ${SELECTED_LANGUAGE} code.`,
                    ``,
                    `${summary ? `## SUMMARY SO FAR:` : ''}`,
                    `${summary ? summary : ''}`,
                ].join('\n').trim()
    ...
    async function isCorrectCode(python_code) {
        ...
        function writeTempCode(code) {
            let tempfile = '';
            switch(SELECTED_LANGUAGE) {
                case 'c' :
                    tempfile = '._tmpcode.c';break;
                case 'java' :
                    tempfile = '._tmpcode.java';break;
                case 'nodejs' :
                    tempfile = '._tmpcode.js';break;
                case 'perl' :
                    tempfile = '._tmpcode.pl';break;
                case 'batch' :
                    tempfile = '._tmpcode.bat';break;
                case 'bash' :
                    tempfile = '._tmpcode.sh';break;
                case 'php' :
                    tempfile = '._tmpcode.php';break;
                case 'python' : default:
                    tempfile = '._tmpcode.py';break;
            }
            fs.writeFileSync(PYTHON_VENV_PATH + '/' + tempfile, code);
            return tempfile;
        }
        if (python_code.trim()) {
            ...
            const tempfile = writeTempCode(python_code)
            let err = await code_validator(`${PYTHON_VENV_PATH}` + '/' + tempfile);
            if (err) {
                rtcode.err = err;
    ...
    async function code_validator(filepath) { // OK
        switch(SELECTED_LANGUAGE) {
            case 'c':
                return ''; // validator 사용 안하고 항상 매뉴 선택 할 수 있도록 수정
                return await code_validator_c(filepath);
            case 'java':
                return ''; // validator 사용 안하고 항상 매뉴 선택 할 수 있도록 수정
                return await code_validator_java(filepath);
            case 'nodejs':
                return ''; // validator 사용 안하고 항상 매뉴 선택 할 수 있도록 수정
                return await code_validator_nodejs(filepath);
            case 'perl':
                return ''; // validator 사용 안하고 항상 매뉴 선택 할 수 있도록 수정
                return await code_validator_perl(filepath);
            case 'batch':
                return await code_validator_batch(filepath);
            case 'bash':
                return await code_validator_bash(filepath);
            case 'php':
                return await code_validator_php(filepath);
            case 'python':
            default:
                return await code_validator_python(filepath);
        }
    }
    async function code_validator_c(filepath) { // OK
        if (isWindows()) {
            let cCmd = `'${c_interpreter}' -c '${filepath}'`;
            let activateCmd = `${PYTHON_VENV_PATH}\\Scripts\\Activate.ps1`;
            return await new Promise(resolve => {
                shelljs.exec(`powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& {& '${activateCmd}'}; ${cCmd}" < nul`, { silent: true }, function (code, stdout, stderr) {
                    resolve(stderr.trim())
                });
            });
        } else {
            let cCmd = `'${c_interpreter}' -c '${filepath}'`;
            return await new Promise(resolve => {
                shelljs.exec(`${bash_path} -c "source ${PYTHON_VENV_PATH}/bin/activate && ${cCmd}"`, { silent: true }, function (code, stdout, stderr) {
                    resolve(stderr.trim())
                });
            });
        }
    }
    async function code_validator_java(filepath) { // OK
        // if (isWindows()) {
        //     let javaCmd = `"${javac_interpreter}" "${filepath}"`;
        //     let activateCmd = `${PYTHON_VENV_PATH}\\Scripts\\Activate.ps1`;
        //     return await new Promise(resolve => {
        //     // console.log(`powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& {& '${activateCmd}'}; ${javaCmd}" < nul`)
        //         // powershell.exe -NoProfile -ExecutionPolicy Bypass -Command 실행시 명령어 사이에 공백 있으면 애러남. 아! 그래서 interpreter에 공백있으면 미사용하도록 했구나..쩝. 
        //         shelljs.exec(`powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& {& '${activateCmd}'}; ${javaCmd}" < nul`, { silent: true }, function (code, stdout, stderr) {
        //             resolve(stderr.trim())
        //         });
        //     });
        // } else {
        //     let javaCmd = `'${javac_interpreter}' '${filepath}'`;
        //     return await new Promise(resolve => {
        //         shelljs.exec(`${bash_path} -c "source ${PYTHON_VENV_PATH}/bin/activate && ${javaCmd}"`, { silent: true }, function (code, stdout, stderr) {
        //             resolve(stderr.trim())
        //         });
        //     });
        // }
    }
    async function code_validator_perl(filepath) {
    }
    async function code_validator_nodejs(filepath) {
        // if (isWindows()) {
        //     let jsCmd = `'${jshint_interpreter}' '${filepath}'`;
        //     let activateCmd = `${PYTHON_VENV_PATH}\\Scripts\\Activate.ps1`;
        //     return await new Promise(resolve => {
        //         shelljs.exec(`powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& {& '${activateCmd}'}; ${jsCmd}" < nul`, { silent: true }, function (code, stdout, stderr) {
        //             resolve(stderr.trim())
        //         });
        //     });
        // } else {
        //     let jsCmd = `'${jshint_interpreter}' '${filepath}'`;
        //     return await new Promise(resolve => {
        //         shelljs.exec(`${bash_path} -c "source ${PYTHON_VENV_PATH}/bin/activate && ${jsCmd}"`, { silent: true }, function (code, stdout, stderr) {
        //             resolve(stderr.trim())
        //         });
        //     });
        // }
    }
    async function code_validator_batch(filepath) {
        // 코드가 있는 것과 코드가 없는겻(실행결과 설명글 같은거)이 ._tmpcode.bat 파일에 함깨 저장되기 때문에 코드가 없으면 애러처리합니다.
        return await new Promise(resolve => {
            fs.readFile(filepath, 'utf8', (err, data) => {
                if (err) { // 애러 있으면 코드 검증 오류 처리
                    resolve(err);
                }
                if(data.indexOf('@echo')>-1) { // @echo off 소스 코드 있으면 코드 검증 성공 처리
                    resolve('')
                } else { // @echo off 소스 코드 없으면 코드 검증 오류 처리
                    resolve('empty source');
                }
            });
        });
    }
    async function code_validator_bash(filepath) {
        // 코드가 있는 것과 코드가 없는겻(실행결과 설명글 같은거)이 ._tmpcode.sh 파일에 함깨 저장되기 때문에 코드가 없으면 애러처리합니다.
        return await new Promise(resolve => {
            fs.readFile(filepath, 'utf8', (err, data) => {
                if (err) { // 애러 있으면 코드 검증 오류 처리
                    resolve(err);
                }
                if(data.indexOf('#!/bin/bash')>-1) { // #!/bin/bash 소스 코드 있으면 코드 검증 성공 처리
                    resolve('')
                } else { // #!/bin/bash 소스 코드 없으면 코드 검증 오류 처리
                    resolve('empty source');
                }
            });
        });
    }
    async function code_validator_php(filepath) { // OK
        // const phpCmd = `'${php_interpreter}' '-l' '${filepath}'`;
        // 코드가 있는 것과 코드가 없는겻(실행결과 설명글 같은거)이 ._tmpcode.php 파일에 함깨 저장되기 때문에 코드가 없으면 애러처리합니다.
        return await new Promise(resolve => {
            fs.readFile(filepath, 'utf8', (err, data) => {
                if (err) { // 애러 있으면 코드 검증 오류 처리
                    resolve(err);
                }
                if(data.indexOf('<?php')>-1) { // <?php 소스 코드 있으면 코드 검증 성공 처리
                    resolve('')
                } else { // <?php 소스 코드 없으면 코드 검증 오류 처리
                    resolve('empty source');
                }
            });
        });
    ...
    async function checkPHP() {
        ...
        if (!php_interpreter) {
            console.error('This app requires php interpreter.')
            process.exit(1)
            return;
        } else if (php_interpreter.indexOf(' ') > -1) {
            print(`* php interpreter found located at "${e}"`);
            print("However, the path should not contain spaces.");
            process.exit(1)
            return;
        }
    ...
    program
        ...
        .option('-l, --language', 'Select Program Langauge. eq) python, php, nodejs, perl, java, c, bash, batch', '')
        ...
        .action(async (prompt, options) => {
            ...
            if (options.python || options.language=='python') {
                try {
                    await checkPython();
                            ...
                            if (index === 4) { // Save Code & Quit
                                ...
                                fs.writeFile(filePath, save_code, (err) => {
                                    if (err) {
                                        ...
                                        print(chalk.gray(`File ${filePath} was saved successfully.`));
                                    }
                                });
                                break;
                            }
                            // copy execute code 
                            _executed_code = python_code
                            // execute
